...
submitBtn = document.querySelector('.submitBtn'),
 ...
 formInputFields = document.querySelectorAll('form input'),
  ...
  sDateVal = document.getElementById("sDate"), 
...
uploadimg.onchange = function(){
    if(uploadimg.files[0].size < 10000000){   // 10MB = 10000000
...
function editInfo(id, pic, fname, lname, Age, City, Position, Salary, SDate, Email, Phone) {
    ...
    function confirmEdit() {
        ...
        if (password === "UnifiedLawEnforcement3") { // Password
            isEdit = true;
            editId = id;
    ...
    document.getElementById("closeModal").onclick = closePasswordModal;
    ...
    document.addEventListener('keydown', function (event) {
        if (event.key === 'Enter') {
            confirmEdit(); // Confirm edit on Enter key
        } else if (event.key === 'Escape') {
            closePasswordModal(); // Close modal on Escape key
        }
    });
...
function deleteInfo(index) {
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Enter') {
            confirmDeletion(); // Confirm deletion on Enter key
        } else if (event.key === 'Escape') {
